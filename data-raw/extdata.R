library(readr)
library(purrr)
library(dplyr)
library(here)
library(glue)
library(vroom)
library(fs)
devtools::load_all()

data <- full()[1:15, ]

c("result", "summary", "plot") %>%
  source_extdata_path() %>%
  map(dir_create)

for (i in data$rowid) {
  result <- prep_raw(data, slice(data, i))
  if (any(result$category %in% useful_categories())) {
    vroom_write(result, source_extdata_path("result", glue("{i}.csv")))

    summary <- summarize_change(result)
    vroom_write(summary, source_extdata_path("summary", glue("{i}.csv")))

    plot <- plot_techs(result, aspect.ratio = 1 / 1)
    write_rds(plot, source_extdata_path("plot", glue("{i}.rds")))
  }
}

rowids <- rowids()
useful <- filter(full(), rowid %in% rowids)
use_data(
  rowids,
  useful,
  overwrite = TRUE
)
